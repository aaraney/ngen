...
double NetCDFPerFeatureDataProvider::get_value(const CatchmentAggrDataSelector& selector, ReSampleMethod m) 
{
    ...
    std::size_t page_c_idx = cache::page_p_idx_to_c_idx(p_idx, cache_line_size);
    std::size_t n_page_accesses = (read_len / cache_line_size) + std::min(read_len % cache_line_size, std::size_t(1));
